package net.bjnzoom2.createtestaddon.block.kinetic.crank_wheel;
...
public class CrankWheelVisual extends KineticBlockEntityVisual<CrankWheelBlockEntity> implements SimpleDynamicVisual {
    ...
    public CrankWheelVisual(VisualizationContext modelManager, CrankWheelBlockEntity blockEntity, float partialTick) {
        ...
        crank = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(isLarge ? CTAPartialModels.LARGE_CRANK_WHEEL_HANDLE : CTAPartialModels.CRANK_WHEEL_HANDLE))
                .createInstance();

        rotateCrank(partialTick);

        rotatingModel = instancerProvider().instancer(AllInstanceTypes.ROTATING, Models.partial(isLarge ? CTAPartialModels.LARGE_CRANK_WHEEL_BASE : CTAPartialModels.CRANK_WHEEL_BASE))
                .createInstance();
